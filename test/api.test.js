...
test("Draw.getSelectedIds", (t) => {
  ...
  t.equal(selected.length, 2, "returns correct number of ids");
  t.notEqual(selected.indexOf(lineId), -1, "result contains line");
  t.notEqual(selected.indexOf(pointId), -1, "result contains point");
  Draw.changeMode("simple_select", { featureIds: [polygonId] });
  const nextSelected = Draw.getSelectedIds();
  t.equal(nextSelected.length, 1, "updates length");
  t.equal(nextSelected[0], polygonId, "updates content");
  t.end();
});
...
test("Draw.set", (t) => {
  ...
  t.equal(drawInstance.length, 3, "return value is correct length");
  const pointId = drawInstance[0];
  const lineId = drawInstance[1];
  const polygonId = drawInstance[2];
  t.equal(Draw.get(pointId).geometry.type, "Point", "point id returned");
  t.equal(Draw.get(lineId).geometry.type, "LineString", "line id returned");
  t.equal(Draw.get(polygonId).geometry.type, "Polygon", "polygon id returned");
  t.equal(Draw.getAll().features.length, 3, "all features loaded");
  ...
  t.equal(nextDrawInstance.length, 1, "return value is correct length");
  ...
  t.equal(deleteSpy.callCount, 1, "Draw.delete called");
  t.deepEqual(
  ...
  t.equal(overlappingDrawInstance.length, 2, "return value is correct length");
  ...
  t.equal(deleteSpy.callCount, 0, "Draw.delete not called");
...
test("Draw.deleteAll when in direct_select mode", (t) => {
  ...
  Draw.deleteAll();
  afterNextRender(() => {
    t.equal(Draw.getMode(), "simple_select", "switches to simple_select mode");
    t.equal(
      Draw.getAll().features.length,
      0,
